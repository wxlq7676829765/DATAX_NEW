...
public class FtpWriter extends Writer {
    ...
    public static class Task extends Writer.Task {
        ...
        /* update by wx 20220105 add ftp的excel模式*/
        @Override
        public void startWrite(RecordReceiver lineReceiver) {
            ...
            try {
                ...
                map.put("job",getFtpClientJson(fileFullPath));
                //①先建立文件，②把数据写入
                UnstructuredStorageWriterUtil.writeToStream(lineReceiver,
//                      outputStream, this.writerSliceConfig, this.fileName,
                        map, this.writerSliceConfig, fileFullPath,
                        this.getTaskPluginCollector());
                //20220408-wx
                //这里需要特殊处理，ftp无法直接写入xlsx，所以需要先在本地写好xlsx，然后再传入ftp里面
               /* if(".xlsx".equals(this.suffix)){
                    this.ftpHelper.upLoad(fileFullPath,this.fileName,this.suffix);
                }*/
            } catch (Exception e) {
                throw DataXException.asDataXException(
        ...
        public JSONObject getFtpClientJson(String fileFullPath){
            JSONObject job = new JSONObject();
            job.put("host",this.host);
            job.put("username",this.username);
            job.put("password",this.password);
            job.put("timeout",this.timeout);
            job.put("port",this.port);
            job.put("path",this.path);
            job.put("protocol",this.protocol);
            job.put("fileName",this.fileName);
            job.put("fileFullPath",fileFullPath);
            job.put("writeMode",this.writerSliceConfig.getString(com.alibaba.datax.plugin.unstructuredstorage.writer.Key.WRITE_MODE));
            job.put("encoding",this.writerSliceConfig.getString(com.alibaba.datax.plugin.unstructuredstorage.writer.Key.ENCODING));
            return job;
        }
