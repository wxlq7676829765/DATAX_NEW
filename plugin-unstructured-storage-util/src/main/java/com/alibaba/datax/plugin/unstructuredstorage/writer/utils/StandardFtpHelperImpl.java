...
public class StandardFtpHelperImpl implements IFtpHelper {
    ...
    public OutputStream getOutputStream(String filePath,String  encoding) {
        try {
            ...
            this.ftpClient.changeWorkingDirectory(new String(parentDir.getBytes(LOCAL_CHARSET),FTP.DEFAULT_CONTROL_ENCODING));
            this.printWorkingDirectory();

            String ftpPath2 = new String(filePath.getBytes(LOCAL_CHARSET),FTP.DEFAULT_CONTROL_ENCODING);
    ...
    public void deleteFiles(Set<String> filesToDelete) {
        ...
        try {
            ...
            for (String each : filesToDelete) {
                ...
                eachFile = each;
                boolean fileExist = isFTPFileExist(eachFile);
                if(fileExist){
                    deleteOk = this.ftpClient.deleteFile(new String(each.getBytes("GBK"), StandardCharsets.ISO_8859_1));
                    if (!deleteOk) {
                        String message = String.format(
                                "删除文件:[%s] 时失败,请确认指定文件有删除权限", eachFile);
                        throw DataXException.asDataXException(
                                FtpWriterErrorCode.SECURITY_NOT_ENOUGH, message);
                    }
                }else {
                    String message = String.format(
                            "没有此路径的文件:[%s]", eachFile);
     ...
     * @author wx
     * @createTime 2023年3月20日 上午11:26:44
     *
    ...
    public boolean isFTPFileExist(String filePath) {
        try {
            ...
            this.ftpClient.enterLocalPassiveMode(); //开启本地被动模式--Linux，windowns开启主动enterLocalActiveMode
            ...
            this.ftpClient.changeWorkingDirectory(new String(dir.getBytes("GBK"),
                    FTP.DEFAULT_CONTROL_ENCODING));

            // 检验文件是否存在
            InputStream is = this.ftpClient.retrieveFileStream(new String(file
                    .getBytes("GBK"), FTP.DEFAULT_CONTROL_ENCODING));
            if (is == null || this.ftpClient.getReplyCode() == FTPReply.FILE_UNAVAILABLE) {
                return false;
            }
    ...
    public boolean isDirExist(String directoryPath) {
        try {
            return ftpClient.changeWorkingDirectory(new String(directoryPath.getBytes("GBK"),FTP.DEFAULT_CONTROL_ENCODING));
        } catch (IOException e) {
